...
class TreeGraphics < Component
  ...
  def draw(viewport)
    if @shaking
      ...
      shaking_x, shaking_y = adjust_shake(center_x, center_y, shaking_for)
  ...
  def trees
    @@trees ||= Gosu::TexturePacker.load_json(Utils.media_path("ground.json"), :precise)
  end
end
